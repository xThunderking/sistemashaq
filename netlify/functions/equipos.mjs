...
const validate = b => !b?.serialNumber?.trim() ? 'El número de serie es obligatorio.' : !b?.area?.trim() ? 'El área es obligatoria.' : !b?.responsable?.trim() ? 'El responsable es obligatorio.' : !ipValid(b?.ipAddress?.trim() || '') ? 'La dirección IP no es válida.' : null;
...
export async function handler(event) {
  ...
  try {
    const db = getPool();
    if (method === 'GET' && !hasId) { const [rows] = await db.execute(`SELECT e.id, e.serial_number AS serialNumber, e.area, e.responsable, e.ip_address AS ipAddress, e.created_at AS createdAt,
      ...
      COALESCE(a.bloqueo_configuracion, 0) AS bloqueoConfiguracion, COALESCE(a.glpi, 0) AS glpi
    ...
    if (method === 'POST') {
      ...
      try { await connection.beginTransaction(); const [r] = await connection.execute('INSERT INTO equipos (serial_number, area, responsable, ip_address) VALUES (?, ?, ?, ?)', [b.serialNumber.trim(), b.area.trim(), b.responsable.trim(), b.ipAddress.trim()]); const a = b.auditoria || {}; await connection.execute('INSERT INTO auditoria_2026 (equipo_id, revision_software, antivirus, usb, paginas_no_autorizadas, escritorio, tiempo_bloqueo, bloqueo_configuracion, glpi) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [r.insertId, !!a.revisionSoftware, !!a.antivirus, !!a.usb, !!a.paginasNoAutorizadas, !!a.escritorio, !!a.tiempoBloqueo, !!a.bloqueoConfiguracion, !!a.glpi]); await connection.commit(); return reply(201, { id: r.insertId, message: 'Equipo registrado.' }); } catch (e) { await connection.rollback(); throw e; } finally { connection.release(); }
    }
    if (method === 'PUT' && hasId) {
      const b = JSON.parse(event.body || '{}'), error = validate(b); if (error) return reply(400, { error });
      const connection = await db.getConnection();
      try { await connection.beginTransaction(); const [r] = await connection.execute('UPDATE equipos SET serial_number=?, area=?, responsable=?, ip_address=? WHERE id=?', [b.serialNumber.trim(), b.area.trim(), b.responsable.trim(), b.ipAddress.trim(), Number(id)]); if (!r.affectedRows) { await connection.rollback(); return reply(404, { error: 'Equipo no encontrado.' }); } const a = b.auditoria || {}; await connection.execute(`INSERT INTO auditoria_2026 (equipo_id, revision_software, antivirus, usb, paginas_no_autorizadas, escritorio, tiempo_bloqueo, bloqueo_configuracion, glpi) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON DUPLICATE KEY UPDATE revision_software=VALUES(revision_software), antivirus=VALUES(antivirus), usb=VALUES(usb), paginas_no_autorizadas=VALUES(paginas_no_autorizadas), escritorio=VALUES(escritorio), tiempo_bloqueo=VALUES(tiempo_bloqueo), bloqueo_configuracion=VALUES(bloqueo_configuracion), glpi=VALUES(glpi)`, [Number(id), !!a.revisionSoftware, !!a.antivirus, !!a.usb, !!a.paginasNoAutorizadas, !!a.escritorio, !!a.tiempoBloqueo, !!a.bloqueoConfiguracion, !!a.glpi]); await connection.commit(); return reply(200, { message: 'Equipo actualizado.' }); } catch (e) { await connection.rollback(); throw e; } finally { connection.release(); }
    }
